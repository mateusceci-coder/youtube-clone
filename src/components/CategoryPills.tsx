    ...
    <div ref={containerRef}  className="overflow-x-hidden relative">
        ...
        style={{ transform: `translateX(-${translate}px)` }}>
            {categories.map(category => (
                <Button onClick={() => onSelect(category)} key={category} variant={selectedCategory === category ? "dark" : "default"} className="py-1 px-3 rounded whitespace-nowrap">{category}</Button>
            ))}
        </div>
